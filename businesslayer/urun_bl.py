...
def urun_ekle(urun_adi,maliyet,satis_fiyat,stok,notlar):
    if not urun_adi or not maliyet or not satis_fiyat or not stok:
    ...
    urun_dal.urun_ekle(urun_adi,maliyet,satis_fiyat,stok,notlar)

def urun_guncelle(urun_id,urun_adi,maliyet,satis_fiyat,stok,notlar):
    if not urun_id or not urun_adi or not maliyet  or not satis_fiyat or not stok:
        raise ValueError('Tüm alanlar doldurulmak zorundadır')
    if float(maliyet)<=0:
        raise ValueError("Maliyet 0'dan büyük olmalıdır")
    if float(satis_fiyat)<=0:
    ...
    if float(stok)<=0:
        raise ValueError("Stok 0'dan büyük olmalıdır")
    urun_dal.urun_guncelle(urun_id,urun_adi,maliyet,satis_fiyat,stok,notlar)
